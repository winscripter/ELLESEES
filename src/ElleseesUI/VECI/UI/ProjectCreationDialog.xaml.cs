...
public partial class ProjectCreationDialog : Window
{
    ...
    private void UpdateNamingStat(string name)
    {
        ...
        if (Path.Combine(Directory.GetCurrentDirectory(), Locations.VECIProjectsFolder, name).Length + 2 > MAX_PATH)
        {
        ...
        if (Directory.Exists(Path.Combine(Directory.GetCurrentDirectory(), Locations.VECIProjectsFolder, name)) ||
            File.Exists(Path.Combine(Directory.GetCurrentDirectory(), Locations.VECIProjectsFolder, name)))
        {
        ...
        _fileName = Path.Combine(Directory.GetCurrentDirectory(), Locations.VECIProjectsFolder, name);
    }
    ...
    {
        OutputPath.Text = $"Will be saved in: {Path.Combine(Directory.GetCurrentDirectory(), Locations.VECIProjectsFolder, path)}";
    }

    /// <summary>
    /// Event handler for the OK button.
    /// </summary>
    protected void OnCreateButtonClick(object sender, RoutedEventArgs e)
    {
        if (!isOkToProceed)
        {
        ...
        Locations.EnsureVECIProjectsFolderExists();

        if (!File.Exists(Path.Combine(_fileName, "project.veci")))
        {
            using var fs = File.Create(Path.Combine(_fileName, "project.veci"));
        }

        try
        {
            using var bw = new BinaryWriter(File.Open(Path.Combine(_fileName, "project.veci"), FileMode.Open), Encoding.UTF8, false);

            bw.Write("ELLESEES VECI");
            bw.Write("data");
            bw.Write(1.0F);
            bw.Write(0);
            ...
            Directory.CreateDirectory(Path.Combine(_fileName, "data"));

            var proj = VECIFile.Load(_fileName);

            var window = new VECIEditor();
            window.LoadVECI(proj);
            window.ShowDialog();
        }
        catch (Exception ex)
        {
